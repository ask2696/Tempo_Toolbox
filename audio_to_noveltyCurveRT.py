...
def noveltyCurveRT(f_audio,Fs,parameter,prev_bandData,frame_num):
    """
    parameter = {};
    parameter['fs'] = Fs
    ...
    for band in np.arange(np.shape(bands)[0]):
                     ...
                     if frame_num == 1:
                         #if band ==0:
                             #prev_bandData = np.zeros([np.shape(bandData)[0],np.shape(bands)[0]])
                             
                         prev_bandData[band]= np.zeros([np.shape(bandData)[0],1])
                         #prev_bandData[0:np.shape(bandData)[0],band] = 0
                         #print np.shape(bandData)
                         #print np.shape(prev_bandData[band])
                         value = np.array(prev_bandData[band])
                     else:
                         value = np.array([prev_bandData[band]]).T


                     #add = np.array([bandData[:,0]]).T

                     #value = np.array(prev_bandData[band])
                     #print band,frame_num
                     #print np.shape(value)
                     #print np.shape(bandData)
                     var = np.hstack((value,bandData))
                     diff = np.diff(var)
                     #print diff
                     #print np.shape(diff)
                     diff = diff*(diff>0)
                     prev_bandData[band] = bandData[:,np.shape(bandData)[1]-1]
    ...
    #plt.plot((np.arange(float(len(NoveltyCurve)))/featureRate),NoveltyCurve)
    #plt.show()
    return (NoveltyCurve,featureRate,prev_bandData)
